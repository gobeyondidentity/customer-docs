...
//    Svg: require('@site/static/img/Details-page-02.svg').default,
    ...
    href: ('/windows-on-premises'),
    label: ('Learn more'),
  },
];

function WindowsDesktopLogin({ title, description, href }) {
  return (
...
export default function WindowsDesktopLoginGuides() {
  return (
    <section className={styles.topics}>
      <Container sx={{ marginLeft: 0, marginTop: 0 }}>
        <div className={clsx('row', styles.topicsRow)}>
          {List.map((props, idx) => (
            <WindowsDesktopLogin key={idx} {...props} />
          ))}
        </div>
