---
title: Universal Passkeys in Secure Workforce
...
1. Generate a passkey creation link using Beyond Identity APIs.

2. Deliver the passkey creation link to the user via email or another method if `RETURN` is specified for the delivery option.

3. When the link is activated, it directs the user to the Platform Authenticator, where a private key is created and securely stored in the device's hardware trust module. Simultaneously, the corresponding public key is saved in the Beyond Identity Cloud.
