---
title: Protect Your SSO
id: protect-your-sso
description: "Learn how to use Beyond Identity as an MFA solution in your Okta Instance."
slug: /auth-and-sso/protect-your-sso
keywords:
  - mfa
  - single device
  - identity provider
  - protect your sso
pagination_next: null
pagination_prev: null
last_update:
  date: 04/03/2024
  author: Ismail Mustafa
draft: false
hide_table_of_contents: false
hide_title: false
hide_breadcrumbs: false
doc_type: overview
displayed_sidebar: mainSidebar
---

Identity Providers allow you to configure authentication using single device MFA. This is essential for enabling your SSO to use Beyond Identity for Authentication.

## Navigate to Protect Your SSO

To access the Identity Provider settings, navigate to **Auth & SSO > Protect Your SSO**. Here you can add and configure a new identity provider.

## Add an Identity Provider

To begin adding an identity provider, click on **Add Identity Provider**. You will be presented with a few options:

![Select Identity Provider](../../../static/img/select-identity-provider.png)

The following identity providers are currently available:

- [Generic OIDC](/docs/auth-and-sso/protect-your-sso/generic-oidc)
