...
In order to enable user syncing between Okta and Beyond Identity, [follow the steps here](/docs/directory/directory-integrations/okta). Return to this guide once you've enabled user syncing between Okta and Beyond Identity.
...
Enable Okta to use Beyond Identity for authentication by setting it up as an identity provider. This is achieved by creating an identity provider within the Beyond Identity Admin Console. Follow the steps here to create a [generic OIDC Identity Provider](/docs/auth-and-sso/identity-providers/generic-oidc). Return to this guide once you've created a generic OIDC Identity Provider in Beyond Identity.
