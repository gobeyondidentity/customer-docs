...
| **Expanded OIDC Claims Handling** | Enhanced the Identity Provider configuration to support claim structures aligned with the OIDC specification, improving compatibility with identity providers like Nametag. PAR fields were added as an enablement flag and endpoint entry for compliant integrations.<br /><br /> |
| **RealityCheck User Interface Redesign** | RealityCheck has been refreshed with a redesigned interface and updated branding, improving usability and providing a more modern and consistent user experience. |
| **New SAML Authentication Methods** *(Limited Availability)* | Authentication methods, such as biometrics, OS verification, and roaming authenticators, are now included in SAML responses and activity logs. Admins can also enforce policy decisions based on the availability of biometric sensors, offering greater visibility and control over authentication strength. This feature is optional; to enable it, please contact our Support team.<br /><br />For full documentation, click here (Link private until release day). |
