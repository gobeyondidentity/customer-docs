...
<details>
  <summary><p>v2.112.0 Beyond Identity Authenticator Release Notes</p></summary>

Beyond Identity began rolling out the new **Platform Authenticator v2.112.0** release on **June 19, 2026**.
...
|----------|---------|-------------|
| **Android** | **Accessibility-Based Sign-In Disclosure** | Added a clear consent/disclosure dialog before enabling the Accessibility Service for automatic sign-in. The dialog explains that the service only detects a sign-in marker the app places itself and never reads, stores, or transmits other on-screen content. The action has been renamed to "Set up automatic sign-in." |
| **Android** | **App Links Aligned with iOS Universal Links** | Android sign-in deep links now use dedicated channel-specific subdomains (`android-auth.*`) aligned with the iOS universal-link infrastructure, instead of root domains, for consistent and reliable deep-link handling across platforms. |
...
|----------|-------------|-------------|
| **Windows** | **Faster Device-Compliance Checks** | Significantly improved Windows authentication speed. Device compliance checks for registry keys and Windows services now complete almost instantly, eliminating a delay of up to one to two seconds per policy rule during sign-in. |
...
|----------|---------|-------------|
| **Windows** | **Continuous Authentication Network Freeze** | Resolved an issue where the Platform Authenticator on Windows could freeze indefinitely during continuous authentication when network conditions change, such as during VPN transitions. It now enforces bounded timeouts on network operations and recovers gracefully instead of requiring a force-quit. Administrators can tune these timeouts via Group Policy DWORD values (`ProxyResolveTimeout`, `ProxyConnectTimeout`, `ProxySendTimeout`, `ProxyReceiveTimeout`) under `HKLM\Software\Policies\BeyondIdentity\Authenticator`. |
| **Windows** | **Startup Crash on Per-Scheme Proxy** | Resolved an issue where the Platform Authenticator on Windows could crash at startup on systems configured with a per-scheme proxy (the format set by tools such as Fiddler, and used by some corporate proxy configurations). The authenticator now interprets these Windows proxy settings correctly, and an unrecognized proxy configuration no longer prevents it from starting. |
| **Windows** | **Startup Crash When Local Port In Use** | Resolved an issue on Windows where the Beyond Identity application could close unexpectedly at startup if its local communication port was already in use. The authenticator now handles an unavailable port gracefully instead of crashing. |
| **Windows** | **Authentication Hang on Unresponsive Fingerprint Driver** | Resolved an issue on Windows where authentication could hang (and sometimes fail) when a fingerprint sensor's driver became unresponsive — for example, certain fingerprint drivers after sleep/resume. Fingerprint availability and verification checks are now bounded by a timeout (default 5 seconds, adjustable by administrators via the `BioNotRespondingSec` policy value), so an unresponsive fingerprint driver no longer blocks sign-in; the authenticator warns the user and continues with other available methods. |
| **Windows** | **Authentication Hang Collecting Device Information** | Resolved an issue on Windows where authentication could hang when collecting certain device details (such as hardware identifiers) if the underlying Windows system-information service was slow or unresponsive. These lookups are now bounded by a timeout, so they can no longer stall sign-in. |
| **Windows** | **BitLocker Disk-Encryption Policy Deny** | Resolved an issue affecting Secure Access where Windows devices with BitLocker enabled could be incorrectly denied by disk-encryption policy. The Platform Authenticator now reports BitLocker drive-encryption status correctly, so policies that require an encrypted system disk evaluate as expected on Windows. |
| **Windows** | **Passkey Deletion Notification** | Resolved an issue on Windows where the notification shown after a passkey is deleted incorrectly displayed "An internal error occurred" alongside a "Report" button. The notification now shows a clear, localized message with only a "Dismiss" button. |
| **Android** | **Passkey List Disappearing After "Add Passkey"** | Closing the Add-passkey flow no longer drops to the welcome/onboarding screen on a transient empty-credentials result. |
| **Android** | **Credential Not Shown After Fresh-Install Passkey Setup** | Fixed an issue on a fresh install where, after setting up a passkey, the app returned to the first screen and did not show the newly created credential. The credential list now updates correctly after enrollment. |
| **Android** | **Home Screen Flickering** | Fixed the welcome/onboarding screen flickering when the device has no passkeys enrolled. |
...
<details>
  <summary><p>Secure Access (Cloud 2.112) Release Notes</p></summary>

Beyond Identity began deployment of a new Secure Access software release on **March 29, 2026**.
...
| Bug Fix | Description |
|--------|-------------|
| **Hosted Web Authenticator Logo Display** | Resolved an issue where tenant logos appeared cropped in the Hosted Web Authenticator. Custom logos now display fully regardless of their aspect ratio. |
| **Hosted Web Authenticator Branding Data** | Resolved an issue where the Hosted Web Authenticator failed to load branding data due to malformed authorization headers. Custom branding — including logos, support URLs, and colors — now displays correctly. |
| **Authentication Screen IE11 Compatibility** | Resolved a JavaScript error in the authentication screen that caused failures on Internet Explorer 11 and legacy WebView environments. Authentication now completes reliably in these environments for all tenants. |
...
Beyond Identity began deployment of a new Secure Access software release on **March 13, 2026**.
...
|-------------|-------------|
| **Improved Accessibility** | The branded authentication loading screen now meets WCAG standards for screen reader compatibility, ensuring accessible authentication experiences for all users. |
| **Support Link Display** | The support link configured in branding settings is now displayed in the branded interstitial for Secure Access tenants, giving end users a direct path to help during authentication. |
| **Improved Windows Webview Rendering** | Resolved rendering issues with the branded interstitial on Windows webview (Internet Explorer), including missing backgrounds, gradients, spacing, and animations. |
| **Preloaded Tenant Logos** | Tenant logos are now preloaded in the branded interstitial, eliminating visual flash during authentication. |
| **Branded Authentication Loading Screen General Availability** | Secure Access now supports branded authentication loading screens across all commercial tenants, enabling consistent branding and messaging throughout authentication flows. Previously available by request only, this feature is now enabled by default. Some tenants have been excluded from automatic enablement pre request. Please submit a ticket with our Support Team to make changes to your configuration. |
...
| Bug Fix | Description |
|--------|-------------|
| **Policy Event Logging Inconsistency** | Resolved an issue where Policy Deny events in the Secure Access console incorrectly displayed "Allowed with Biometric Verification" as the matched rule. |
| **Mobile Passkey Display** | Corrected long text being cut off on the mobile passkey-in-browser selection screen. |
| **iPhone Safari Extension Suggestion** | Fixed Safari extension suggestion text incorrectly appearing in Chrome, Firefox, and Edge browsers on iPhone. The prompt now only displays in Safari. |
| **Loading Spinner Alignment** | Fixed the branded interstitial loading spinner not being left-aligned on desktop screens. |
| **Branded Interstitial Safari Spinner** | Fixed the branded interstitial loading spinner not appearing when using the Safari extension. |
...
<details>
  <summary><p>Beyond Identity RealityCheck for Zoom (Secure Access) Release Notes</p></summary>
...
| **Enhancement** | **Description** |
|-----------------|-----------------|
| **Improved Zoom Badge Display for Long Job Titles** | Long job titles are now displayed correctly on the Zoom badge, preventing truncation or layout issues and ensuring accurate identity representation. |
| **Last Used Company Identifier Is Now Remembered** | The login page now saves the last used company identifier, making repeat sign-ins faster and more convenient for returning users. |
| **Autocorrect Disabled for Company Identifier Field** | Autocorrect has been removed from the company identifier field on the login page to prevent unintended text changes and login errors. |
| **Corrected Last Authentication Date Display** | An issue causing the last authentication date to display incorrectly has been fixed, ensuring accurate and reliable audit information. |
...
<details>
  <summary><p>v2.110.0 Beyond Identity Authenticator Release Notes</p></summary>

Beyond Identity began rolling out the new **Platform Authenticator v2.110.0** release on **February 10, 2026**.
...
| Enhancement | Description |
|-------------|-------------|
| **Enable Microsoft Smart Card Sign-In for Windows Desktop** | Administrators can now enable the Microsoft Smart Card sign-in option on Windows Desktop Login via a registry setting, giving organizations more flexibility in how users authenticate. |
| **iOS Safari Extension Compatibility** | Improved compatibility and reliability when authenticating using the iOS Safari Extension with the Beyond Identity Authenticator. |
...
| Bug Fix | Description |
|--------|-------------|
| **Android Platform Authenticator UI Issue** | Fixed an issue where the Android Platform Authenticator incorrectly displayed a **REPORT** button instead of **DISMISS** on customer-facing screens. |
| **Layered Authentication QR Fix (Android)** | Fixed an issue where Layered Authentication did not complete when scanning the QR Code with the native camera application on Android. |
...
## What’s New
<br/>
**RealityCheck User Interface Refresh**
<br/>
RealityCheck for Zoom has been refreshed with a redesigned interface and updated branding, improving usability and providing a more modern, consistent, and secure user experience.
<br/>
For full RealityCheck for Zoom documentation, [click here](https://docs.beyondidentity.com/docs/access-control/applications/realitycheck-for-zoom).

<br/>


    ![RealityCheck](../images/realitycheck-zoom-1.png)
...
Beyond Identity began deployment of a new Secure Access software release on February 1, 2026. 

## What's New

| Feature | Description |
|--------|-------------|
| **Authentication Loading Screen**<br /><br />*(Limited Availability)* | Secure Access now supports branded authentication loading screens, allowing organizations to display customized branding and messaging during authentication.<br /><br />Please submit a ticket with our Support Team to enable this feature for your tenant.<br /><br />For full documentation, [click here](https://docs.beyondidentity.com/docs/tools/branding) |
| **Splunk App Integration** | Secure Access is now supported by the Beyond Identity App for Splunk, providing enhanced monitoring and visibility into authentication events. For full documentation, [click here](https://docs.beyondidentity.com/docs/tools/integrations/splunk) |
...
| Enhancement | Description |
|------------|-------------|
| **Improved Page Responsiveness**<br /><br /> | Improved layout responsiveness to enhance usability across various devices and screen sizes. |
...
| Bug Fix | Description |
|--------|-------------|
| **Secure Access Console Page Crash** | Resolved an issue that could cause a page crash within the Secure Access Console. |
| **macOS Safari Authentication Issue** | Fixed an authentication issue affecting macOS users running Safari version 26.2. |
| **General Stability Improvements** | Applied multiple minor fixes to improve overall stability and reliability. |
...
<details>
  <summary><p>v2.109.0 Beyond Identity Authenticator Release Notes</p></summary>

Beyond Identity began deployment of a new version of its platform Authenticator on January 5, 2026. 

### Enhancements

| Platform | Feature Description |
| --- | --- |
| iOS | **Updated Safari Extension Instructions for iOS 26** — The Safari extension setup instructions have been updated to match the latest interface changes introduced in iOS 26, ensuring users receive accurate guidance during extension activation. [For full documentation, click here.](https://docs.beyondidentity.com/docs/end-user-guides/safari-extension) |
...
| Platform | Bug Fix Description |
| --- | --- |
| Android | **Corrected REPORT/DISMISS Button Behavior on Android** — Fixed an inconsistency where Android displayed a REPORT button on certain authentication error screens while other platforms showed DISMISS. Android now behaves consistently across all platform authenticators. |
| Android | **Resolved ANR Caused by Main-Thread Background Task** — Addressed an issue where a background operation was incorrectly running on the main UI thread, causing an “Application Not Responding” (ANR) error on Android. The fix improves start-up reliability and overall stability. |
...
Beyond Identity began deployment of a new Secure Access software release on December 6, 2025. 
...
|---------|---------------------|
| **Updated Login Page Background** | A new background color has been introduced on the Secure Access login page to ensure visual consistency across our ecosystem. The background is now white, aligning Secure Access with the existing look and feel of Secure Work.<br /><br />![Login page background change](../images/new-login-page-color.png) |
| **Granular Roaming Authentication Configuration** *(General Availability)* | Secure Access now supports granular configuration for roaming authentication. Administrators can define allowed source IP addresses, users, and user groups in addition to existing realm-level enablement. This enhancement offers finer control over authentication behavior for roaming users.<br /><br />For documentation, <a href="https://docs.beyondidentity.com/docs/authentication-experience/roaming-authentication/roaming-authentication-config">click here</a>. |
| **PAR and Claims Support for Identity Providers** | Added new fields for Identity Providers to support Pushed Authorization Requests (PAR) and additional Claims. This enables admins to configure PAR endpoints and request richer identity information directly from upstream providers.<br /><br />For full documentation, <a href="https://docs.beyondidentity.com/docs/access-control/identity-providers/generic-oidc">click here</a>. |
| **Expanded OIDC Claims Handling** | Enhanced the Identity Provider configuration to support claim structures aligned with the OIDC specification, improving compatibility with identity providers such as Nametag. PAR fields were added as an enablement flag and endpoint entry for compliant integrations.<br /><br /> |
| **New SAML Authentication Methods** *(Limited Availability)* | Authentication methods, including biometrics, OS verification, and roaming authenticators, are now included in SAML responses and activity logs. Admins can also enforce policy decisions based on the availability of biometric sensors, offering greater visibility and control over authentication strength. This feature is optional; to enable it, please contact our Support team.<br /><br />For full documentation, <a href="https://docs.beyondidentity.com/docs/access-control/identity-providers/saml-methods">click here</a>. |
...
| Bug Fix | Description |
|---------|-------------|
| **Backend Support for Correlation IDs on Flow Type Updates** | A backend issue was resolved to ensure correlation IDs are properly generated and associated with **Activity** section entries when flow types are updated. This fix prepares the system for future console-driven flows where correlation IDs will appear automatically to improve traceability. |

**Important**: An issue affecting the Secure Access Platform **Activity View** will be addressed in the next maintenance window.
...
<details>
  <summary><p>Version 2.108.0 BI Authenticator Release Notes</p></summary>

Beyond Identity began rolling out the new Platform Authenticator v2.108.0 release on November 12, 2025.  
...
| --------- | ------------ |
| **Android** | Updated the app to support 16 KB memory page sizes in line with Google Play policy. This optimization delivers faster load times, smoother performance on modern devices, lower power usage during startup, and full compatibility with the latest Android hardware. |
| **Android** | Standardized collection of Android OS version and API level across Secure Access and Secure Work, ensuring consistent and accurate device reporting. |
| **Android** | Improved **Accessibility Settings** detection for Android 13 and later, resolving cases where the Accessibility Service failed to activate on newer devices. |
| **macOS, iOS, Windows, Android** | Added support for QR code and generated-link launches that open the Authenticator directly for credential import, streamlining setup across all platforms. |
...
| Platform | Description |
| --------- | ------------ |
| **Android** | Fixed a tapjacking vulnerability, ensuring users are protected from malicious overlay attacks on Android devices. |
| **iOS** | Fixed an issue where policy data wasn’t properly collected during Layered Authentication transactions, improving policy accuracy and reliability. |
...
<details>
  <summary><p>Version 2.108.0-macOS BI Authenticator Release Notes</p></summary>
...
| PLATFORM | DETAILS |
| --- | --- |
| **macOS** | **Added support for QR-code and generated-link launches** that open the Authenticator directly for credential import, streamlining setup across all platforms. |
| **macOS** | **Improved policy version matching for macOS applications in Secure Work.** Policy rules now accurately evaluate version numbers (including build identifiers), allowing administrators to enforce compliance for apps like Zoom and AWS VPN. |
...
| PLATFORM | DETAILS |
| --- | --- |
| **macOS** | Fixed an issue where the Authenticator installer (PKG) caused the keep-alive launch agent to enter a crash loop, ensuring stable background operation. |
| **macOS** | Fixed an issue where platform icons were missing on the Platform Authenticator downloads page, restoring visual consistency and clarity. |
| **macOS** | Fixed a rare crash that occurred when the Authenticator launched with a URL copied to the clipboard and the user was not part of the admin group, improving overall stability. |
...
| Windows | Enhancement | Changed Azure login when enrolling in Desktop Login to use WebView2. To revert to use older WebView, add the string registry value AzureWebViewType to the registry key HKLM\Software\Policies\BeyondIdentity\Authenticator and set the value to "WebBrowser". |
