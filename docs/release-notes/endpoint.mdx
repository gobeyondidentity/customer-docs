...
|-------------|-------------|
| **Custom Error Messaging for Self-Remediation** | Administrators can now configure custom error messages with a descriptive error title and optional link buttons. End users will see these error messages during registration and authentication on a polished, restyled UI. Settings can be configured in the Secure Work Admin Console. |
| **Self-Remediation via Credential Extension (GA)** | The self-remediation via credential extension feature is now in General Access and available to all tenants. Administrators can enable a setting to allow end users to self-remediate when they do not have a valid passkey on their device by extending an existing passkey. This guided walkthrough on error is disabled by default and can be toggled per error in the Secure Work Admin Console. |
| **Branded Interstitial Loading Screen (GA)** | The branded interstitial feature is now in General Access and has been enabled for all tenants, except for those that have chosen to opt out. |
...
| Bug Fix | Description |
|--------|-------------|
| **Policy Evaluation UI Consistency Fix** | Fixed a UI inconsistency on the policy evaluation page where the "ALLOW - with biometrics" rule incorrectly displayed as "DENY" when users did not provide biometrics. |
