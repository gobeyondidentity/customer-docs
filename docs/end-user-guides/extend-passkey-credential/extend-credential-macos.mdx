---
title: How to Extend Your Passkey Credential to a macOS Device
id: extend-credential-macos
description: 'Learn how to extend your passkey to a macOS device.'
slug: /end-user-guides/extend-passkey-credential/extend-credential-macos
keywords:
  - Extend passkey credential to macOS
  - Passkey
pagination_next: null
pagination_prev: null
last_update:
  date: August 4, 2025
  author: Dolores M. Bernal
doc_type: overview
displayed_sidebar: mainSidebar
---

## Overview

This article is intended for users who have already registered a Beyond Identity passkey on a device and wish to extend it to a macOS device, such as a MacBook, iMac, or another Apple computer.

By completing this process, you'll create a backup of your passkey, making it possible to restore it if it's ever deleted from the original device.

---

### What You'll Learm
This article will walk you through the process of adding or extending your passkey credential to a trusted macOS device. 


---

### Steps

1. If you haven’t already, download the Beyond Identity Authenticator for macOS from https://app.byndid.com/downloads.

    ![Extend](../images/1-extend-to-macos.png)


2. Next, follow the steps to install the Authenticator on your macOS device. 

<img
  src={require('../images/2-extend-to-macos.png').default}
   alt="Authenticator"
      style={{
        maxHeight: '300px',
        width: 'auto',
        objectFit: 'contain',
        display: 'block',
        marginLeft: '2rem',
        marginTop: '1rem',
        marginBottom: '1rem'
      }}
/>


3. Once installed, at the welcome screen, click **Next**. 

    ![Extend](../images/3-extend-to-macos.png)


4. Click **Enroll with Existing Device**. 

     ![Extend](../images/4-extend-to-macos.png)


5. Click **Generate QR / 9 Digit Code**. 

    ![Extend](../images/5-extend-to-macos.png)

When the QR or 9-digit code appear in the next screen, you have only 60 seconds to enter them in the device that already has the passkey installed. 

<img
  src={require('../images/6-extend-to-macos.png').default}
   alt="Authenticator"
      style={{
        maxHeight: '500px',
        width: 'auto',
        objectFit: 'contain',
        display: 'block',
        marginLeft: '2rem',
        marginTop: '1rem',
        marginBottom: '1rem'
      }}
/>



6. For this use case, let's imagine that you want to extend the passkey from your Android device.

    Open the Beyond Identity Authenticator on your Android, then click **Set up other devices**. 

    **NOTE**: The steps may be similar if you are extending the passkey from your iOS (iPhone) device.

<img
  src={require('../images/7-extend-to-macos.png').default}
   alt="Authenticator"
      style={{
        maxHeight: '500px',
        width: 'auto',
        objectFit: 'contain',
        display: 'block',
        marginLeft: '2rem',
        marginTop: '1rem',
        marginBottom: '1rem'
      }}
/>


7. Enter the 9-digit code generated in your macOS machine (see Step 5 above) to the Android device, **OR** tap **Open Camera** to scan the QR code. 

     ![Extend](../images/8-extend-to-macos.png)


8. If your macOS device complies with your organization's security policies, a confirmation banner will appear indicating that the passkey was added successfully.

    You should also see that the passkey is now installed on your macOS device.

     ![Extend](../images/9-extend-to-macos.png)

    If your macOS doesn't meet the security policies, you will see a **Policy Denied** message. Contact your IT admin for assistance. 

     ![Extend](../images/10-extend-to-macos.png)

    That's it. 