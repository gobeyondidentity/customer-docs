...
You can use the filter above the graphs or select an option in the Total Devices widget to quickly locate devices that haven't yet added the enrollment code to begin collecting data or to find the date and time a query last ran on devices that are enrolled. In some cases, the date/time of the last query may vary based on whether the device is offline, etc. 
...
- Pending - Device is not yet enrolled. 
  Note: The enrollment code expires after 10 minutes so you may need to generate a new code for the device. For more information, see <ins>[Generate a new enrollment code](install-authenticator#generate-a-new-enrollment-code)</ins>
...
You can use the filter above the graphs or select an option in the Misconfigurations graph to filter for devices that contain a misconfiguration. 
Coming soon
