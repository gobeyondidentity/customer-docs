...
:::note
In order to see devices, you must first enroll the device by adding a Device360 credential to the Beyond Identity Authenticator version 2.91.1 or later. Note that this is a different passkey than the one used for the Beyond Identity console and passwordless authentication. For more information, see <ins>[Install the authenticator and enroll devices](#Install-the-authenticator-and-enroll-devices)</ins> above.
:::

![Devices table](../images/devices/device-table.png)

The Device Inventory table shows device details and vulnerabilities from polling the device based on the data collection setting.
...
|**Last Response**       | The last time the authenticator responded to a device query. Available values are: <ul><li>Pending - Awaiting a response from the device.</li><li>Active - The date and time of last response to a query.</li></ul>|
