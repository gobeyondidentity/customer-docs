...
Use this filter to quickly locate devices that haven't yet added the enrollment code to begin collecting data or to find the date and time a query last ran on devices that are enrolled. In some cases, the date/time of the last query may vary based on whether the device is offline, etc. 
Available options for this filter are: 
