...
5. Remove the current passkey from the authenticator. For more information, see [Remove a passkey from a device](https://docs.beyondidentity.com/docs/docs/remove-passkey).
...
6. Add the regenerated enrollment code to bind your device to this tenant: 
   ...
   b. Paste or type the enrollment code in the Enter 9-digit code screen. Note that a cursor doesn't appear on the screen. 
      ...
      The device will be added to the Device table with a status of Pending until internal queries complete. Once those complete, the status will change to Active. For more information, see [Analyze devices](https://docs.beyondidentity.com/docs/docs/analyze-devices).