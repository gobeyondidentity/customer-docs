...
This is an overview of the integration for sending Beyond Identity events data to Elastic.
...
🟠 Elastic: Elastic platform tasks are highlighted in orange.

## Stage I

The sections in this stage are for the intial setup of the integration between the two platforms.

### 🟠 Set up Elastic account and custom HTTP endpoint logs
...
### 🟠 Create agent policy in Elastic
...
### 🟠 Add HTTP logs integration to policy

Enter “0.0.0.0”  for the Listen Address and a value for the listen port, for example 8787.
...
3. Select **Elastic**.
4. On the Elastic overview page, click **Add instance**.

## Stage II

The sections in this stage are for setting up the Elastic configuration in Beyond Identity and viewing the logs. Follow the sections below to complete the integration.

### 🔵 Configure Elastic integration in Beyond Identity
...
1. On the **Add integration** pop up after completing the steps in Add integration in Beyond Identity section, input the following required values.
    ...
    - Host → Your Elastic host name
    - Port → Your Elastic custom endpoint HTTP port
