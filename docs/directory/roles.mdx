...
## Manage Roles
The **Roles** feature allows administrators to restrict identity permissions for performing privileged actions in the Secure Access admin console. For example, administrators can define a group called "Help Desk Admins" with permissions to perform limited privileged actions, like sending enrollment emails but not deleting identities.
