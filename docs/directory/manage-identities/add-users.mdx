...
### User Enrollment Status Definitions

**Invite Sent**  
An enrollment invitation was sent to the user, but they haven’t started the process yet.  

**Invite Failed**  
The enrollment invite is invalid or undeliverable. This can happen if:  
- The 7-day enrollment link expired  
- The invite was revoked by the administrator  
- The system failed to send the invite due to an invalid email address or something else  

**Pending**  
The user received the invite and started the enrollment process, but hasn’t completed it yet.  

**Enrolled**  
The user successfully completed the enrollment process and is now onboarded.  

**Unenrolled**  
The user was previously enrolled but has been removed, either manually or through policy.
...
When adding passkeys, you can select and enroll up to 200 users at a time. This bulk enrollment option is a time-efficient way to manage larger groups of users.
