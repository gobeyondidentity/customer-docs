...
In certain scenarios, an IT administrator may need to revoke an identity's passkey to maintain the integrity and security of the organization’s access controls. 

Reasons why it may be necessary to revoke an identity's passkey:
...
- Revoking an identity's passkey 
- Adding a new identity's passkey
