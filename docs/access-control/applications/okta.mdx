...
The Okta SCIM implementation requires a bearer token. Use the Okta platform to complete the SCIM to map users and attributes to Beyond Identity for the integration. Complete the sections in the [Generic SCIM guide](/docs/access-control/applications/generic-scim) for configuring users from the Okta upstream directory source.
