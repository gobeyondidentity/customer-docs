...
:::note
Identities may not appear in the group when pushing the SCIM transfer if users are provisioned before the group in Okta because of a provisioning error during the SCIM transfer. Provision users in Okta after provisioning the group for the SCIM transfer to display users and groups in the correct order. 
<br></br>
If an identity provisioining error occurs during the SCIM transfer, navigate to the **Identities** section of your Okta dashboard to resolve the error.
:::
