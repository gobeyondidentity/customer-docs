...
After configuring Microsoft Entra ID, use the 🔵 Beyond Identity console to assign an identity.
...
Troubleshooting Item #1: User is not being presented with the option for EAM when logging into Microsoft protected resources.

Likely cause #1: Object ID on user's Entra account does not match the External ID on the Beyond Identity account
