...
- Configure new conditional access policy
...
7. Paste the **Discovery Endpoint value** from the **Client Information & Endpoints** of [Step 10 of the previous section](#-set-up-beyond-identity-console) into the **Redirect URI** text field. This is the **Issuer URI** value.
8. Click **Register**.
...
The steps below cover setting up important values in 🟠 Microsoft for Beyond Identity validation after registering the new application.
...
4. On the left-hand navigation under **Manage**, click **Security** → **Authentication methods**.
...
:::note
Before continuing with the integration, confirm that the user `objectid` value in Microsoft EAM maps to the `externalid` value in Beyond Identity.
:::
